...
You may want to specify dedicated scripts for individual cluster jobs, which takes precedence over the global workflow environment script. For instance, the following defines a tool as environment script for the `correctGcBias` cluster job (which is also defined as tool).
